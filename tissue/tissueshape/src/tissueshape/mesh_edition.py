...
def divide_edge (mesh, pos, eid, point, axis, ratio_min = 0.2) :
	"""Divide an edge
...
def divide_face (mesh, pos, fid, point, axis, ratio_min = 0.2) :
...
def divide_cell (mesh, pos, cid, point, axis, ratio_min = 0.2) :
	...
	#create lineage
	lineage = [OrderedDict() for i in xrange(mesh.degree() + 1)]
	# Fred Note: OrderedDict makes it possible to remenber the order of key insertion
	# it is important since id can be reused. And thus a reused id should be stored after its deletion.
	...
	for eid in tuple(mesh.borders(3,cid,2) ) :
		if len(set(dot(pos[pid] - point,axis) > 0 \
		            for pid in mesh.borders(1,eid) ) ) > 1 :
			edge_lineage = divide_edge(mesh,pos,eid,point,axis,ratio_min=ratio_min)
			lineage[0][None].extend(edge_lineage[0][None])
			lineage[1][eid] = edge_lineage[1][eid]
