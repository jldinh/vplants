...
class TissueFile (object) :
	...
	def __init__ (self, filename, mode = 'r') :
		...
		:Parameters:
		 - `filename` (str) - name of the file in
		      which to store or read informations
		 - `mode` (str) - either 'r' to read or
		      'w' to write.
		
		.. note:: Do not create this object directly,
		          use :func:`topen` instead.
		"""
	...
	def filenames (self) :
		...
		Iterator on all name of files actually stored
		in this tissuefile object.
		"""
	...
	def chmod (self, filename, mod = 0644) :
		...
		:Parameters:
		 - `filename` (str) - name of the file
		   whose permissions have to be modified
		 - `mod` (int) - numerical description
		    of the permission (linux type)
		"""
	...
	def set_compression (self, filename, comp = ZIP_DEFLATED) :
		...
		Default is ZIP_DEFLATED. Use this function if you
		need to change it.
		"""
		info = self._elms[filename][0]
	...
	def read_property (self, property_name) :
		...
		:Parameters:
		 - `property_name` (str) - name of the
		    property to read
		
		:Return: property and description
		:Returns Type: dict,str
		"""
		return self.read_dict("%s.tip" % property_name)
	
	def properties (self) :
		"""Iterator on the name of all properties
	...
	def read_tissue (self) :
		"""Read the tissue structure stored in
		the archive
		
		:Return: tissue and description
	...
	def read (self, name=None) :
		"""Magic function to read either
		the tissue if name is None or a
		property if name is given
		
		:Parameters:
		 - `name` (str) - if None read the tissue
		                - else read property
		
		:Return: tissue or property, description
		...
		"""Iterator on the name of all configurations
		stored in this archive.
	...
	def external_files (self) :
		"""Iterator on the name of all external files
		stored in this archive.
	...
	def write_property (self, prop, property_name, description="") :
		...
		:Parameters:
		 ...
		 - `property_name` (str) - name of the file to use
		                  to later retrieve this info
		 - `description` (str) - textual description
		                  of the property
		"""
	...
	def write_tissue (self, tissue) :
		...
		Only one tissue can be stored, so there is no
		need to provide a name to later retrieve the
		tissue.
	...
	def write (self, obj, name=None, description="") :
		"""Magic function to store either a tissue
		or a property in the archive
		
		:Parameters:
		 - `obj` (:class:`Tissue` or dict) - object
		                to store
	...
	def write_config (self, config, config_name) :
		...
		:Parameters:
		 - `config` (:class:Config) - configuration
		                              to store
		 - `config_name` (str) - name to use to retrieve
...
def topen (filename, mode='r') :
	...
	Store or read informations in
	a file depending on the mode.
	
	:Parameters:
	 - `filename` (str) - name of the file
	 - `mode` (str) - either 'r' to read or
	              'w' to store informations
	
	:Returns Type: :class:`TissueFile`
	
	.. note:: Rather use this instead of the
	      constructor of :class:`TissueFile`
	"""
