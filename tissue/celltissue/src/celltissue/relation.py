...
from openalea.container import IdDict,Graph,Relation,Topomesh
...
class GraphRelation (TissueRelation,Graph) :
    ...
    ########################################################
    #
    #        Update after division
    #
    ########################################################
    def division_update(self,_lineage) :
        """Updates graph after cell division.
        """
        _tissue =  self.tissue()
        _mesh_id = _tissue.relations_name("mesh")
        _mesh = _tissue.relation(_mesh_id)
        _dim = _mesh.degree()
        for _deg in xrange(_dim+1) :
            if _mesh._degree_type[_deg] == self._vertex_type :
                _vertex_dim = _deg
            if _mesh._degree_type[_deg] == self._edge_type :
                _edge_dim = _deg
            elif self._edge_type is None :
                _edge_dim = -1
        if _edge_dim == -1 :    # TODO: Implement graphs with no _edge_type
            return
        else :
            for old_vid,vids in _lineage[_vertex_dim].iteritems() :
                for vid in vids :
                    for eid in _mesh.borders(_vertex_dim,vid,_vertex_dim-_edge_dim) :
                        if _mesh.nb_regions(_edge_dim,eid) == 2 :
                            vid1,vid2 = _mesh.regions(_edge_dim,eid)
                            self.add_edge(vid1,vid2,eid)
        return
...
class RelationRelation (TissueRelation,Relation) :
    ...
    ########################################################
    def division_update(self,_lineage) :    
        """Updates relation after cell division.
        """
        _tissue =  self.tissue()
        _mesh_id = _tissue.relations_name("mesh")
        _mesh = _tissue.relation(_mesh_id)
        _dim = _mesh.degree()
        for _deg in xrange(_dim+1) :
            if _mesh._degree_type[_deg] == self._left_type :
                _left_dim = _deg
            if _mesh._degree_type[_deg] == self._right_type :
                _right_dim = _deg
        for old_rid,rids in _lineage[_right_dim].iteritems() :
            for rid in rids :
                lids = _mesh.regions(_right_dim,rid,_left_dim-_right_dim)
                for lid in lids:
                    self.add_link(lid,rid)   
        return
