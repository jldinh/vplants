...
class InvalidElementName (TissueError,KeyError) :
	"""Raised when a wrong element name has been provided
	"""
...
class Tissue (object) :
	...
	def relations_name (self, name) :
		"""Iterate on all relations involving the given name
		
		if `name` is None, iterate on all
		relations maintained by the tissue.
		
		:Parameters:
		 - `type_name` (name) - name of type of relation
		...
		if name is None :
			...
			for _id in self.relations() :
				if self.relation(_id).name == name :
					return _id
			raise InvalidElementName(name)
	...
	def add_relation (self, relation_type="graph",
		...
		try :
			...
			raise InvalidRelation("%s unknown, available relation are : %s" \
			       % (str(relation_type),
