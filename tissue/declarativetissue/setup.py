...
pkg_root_dir = 'src/openalea'
pkgs = [ pkg for pkg in find_packages(pkg_root_dir) if namespace not in pkg]
top_pkgs = [pkg for pkg in pkgs if len(pkg.split('.')) < 2]
packages = [ namespace + "." + pkg for pkg in pkgs]
package_dir = dict( [('','src'),('openalea',pkg_root_dir)] + [(namespace + "." + pkg, pkg_root_dir + "/" + pkg) for pkg in top_pkgs] )


# Meta information
long_description= '''
Libray to manipualte a meristematic structure with declaratve rule.
'''
...
setup(
    ...
    entry_points = {
            'wralea': [],
            'console_scripts': ['mpy = openalea.declarativetissue.mpy:main',]
        },
