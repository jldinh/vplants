...
from vplants.plantgl.scenegraph import Material,Scene,Shape
from openalea.pglviewer import SceneView
from prop_display import (draw_scalar_prop,
                          draw_vectorial_prop,
                          draw_tensorial_prop,
                          _mat_map_func)
...
class ScalarPropView (SceneView) :
	"""View on a scalar prop defined on a mesh.
	"""
	def __init__ (self, mesh, pos, deg, prop, shrink, cmap) :
	...
	def redraw (self, send_signal = True) :
		if self._cache_geometry is None :
			sc = draw_scalar_prop(self._mesh,
				                  self._pos,
				                  self._deg,
				                  self._prop,
				                  self._shrink,
...
class VectorialPropView (SceneView) :
	"""View on a vectorial prop defined on a mesh.
	"""
	def __init__ (self, mesh, pos, deg, prop, scaling, mat) :
		SceneView.__init__(self)
		self.idmode = GLRenderer.ShapeId
		
		self.set_name("tensorial_prop")
	...
	def redraw (self, send_signal = True) :
		sc = draw_vectorial_prop(self._mesh,
		                         self._pos,
		                         self._deg,
		                         self._prop,
		                         self._scaling,
...
class TensorialPropView (SceneView) :
	"""View on a tensorial prop
	"""
	def __init__ (self, topo, pos, elm_type, prop, scaling) :
		SceneView.__init__(self)
		self.idmode = GLRenderer.ShapeId
	...
	def redraw (self, send_signal = True) :
		sc = draw_tensorial_prop(self._topo,
		                         self._pos,
		                         self._elm_type,
		                         self._prop,
		                         self._scaling)
