...
# Meta information
long_description= '''
contains a set of functions that manipulate tissue
properties
'''
...
rc_file = "src/%s/icons/%s.qrc" % (package,package)
out_file = "src/%s/%s_rc.py" % (package,package)
os.system("pyrcc4 -o %s %s" % (out_file,rc_file) )

ui_file = "src/%s/tissue_prop_dialog.ui" % package
out_file = "src/%s/tissue_prop_dialog_ui.py" % package
f = open(out_file,'w')
...
setup(
    ...
    entry_points = {
            'wralea': ['tissueedit = openalea.tissueedit_wralea']
        },
