...
axis = array( (1,0,0) ) #fiber direction
E1 = 1e10 #(Pa) #Young's modulus in main direction
E2 = 10e10 #(Pa) #Young's modulus in fiber direction
...
Fmax = 1e-5 #(N) #force threshold below which points are considered not moving
...
for fid in mesh.wisps(2) :
	...
	#mat
	lv = fr.local_vec(axis)[:2]
	alpha = norm(lv)
	lv /= alpha
	
	lE2 = E1 ** (1 - alpha) * E2 ** alpha
	lG = sqrt(E1 * lE2)
	mat = axis_material2D(lv,E1,lE2,nu,lG)
...
for fid,sp in spring.iteritems() :
	...
	stvec[fid] = [fr.global_vec(v[:,i]) * w[i] for i in inds]
...
#
###################################################
#begin stress distribution
fig = figure()
hist([sp.stress(state_equ).trace() for sp in spring.itervalues()])
fig.savefig("stress_dist.png")
show()
close(fig)
