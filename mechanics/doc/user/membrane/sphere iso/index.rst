.. _meca_membrane_sphere_iso:

#################################
Sphere with isotropic material
#################################
...
The goal of this document is to reproduce the isotropic sphere (:ref:`meca_basics_mechanics_lib`) using membrane elastic elements instead of linear ones. The python script for this example can be downloaded (download file: :download:`simu.py`) and run in a shell console using::
...
From an anlytical point of view, we expect the norm of the strain to be uniform equal to 0.01.

.. image:: strain_dist.png
    :width: 300pt
    :align: center

The error observed between the analytical result and the distribution observed above is due mainly to:

 - the fact that the geometry of the sphere is approximated using an icosaedre
 - the threshold chosen to decide the convergence of the algorithm
