...
Finally we can run the lineaging algorithm::
    ...
    from vplants.mars_alt.alt.all import full_alt_loop
    ...
    final_map, reason, scores = full_alt_loop(imseg_t0, imseg_t1, expert_mapping, ts0, ts1, mat_init=trsf_vox, filter_params=filters)

The **full_alt_loop** function will iteratively try to converge to a stable mapping by performing
