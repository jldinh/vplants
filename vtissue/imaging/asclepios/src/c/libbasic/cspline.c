					   ...
					   )
{
  ...
  */
  ...
  for ( z=0; z<dimz; z++ ) {
	...
	return( -1 );
      case UCHAR :
	ConvertBuffer( tmpBuf, FLOAT, &((u8*)bufferOut)[z*dimx*dimy], 
		       typeOut, dimx*dimy );
	break;
      case SCHAR :
	ConvertBuffer( tmpBuf, FLOAT, &((s8*)bufferOut)[z*dimx*dimy], 
		       typeOut, dimx*dimy );
	break;
      case USHORT :
	ConvertBuffer( tmpBuf, FLOAT, &((u16*)bufferOut)[z*dimx*dimy], 
		       typeOut, dimx*dimy );
	break;
      case SSHORT :
	ConvertBuffer( tmpBuf, FLOAT, &((s16*)bufferOut)[z*dimx*dimy], 
		       typeOut, dimx*dimy );
      }
    }
  }
			      ...
			      int *derivative )
{
  ...
  for ( z = 0; z < resDim[2]; z ++ ) {
	...
	return( -1 );
      case UCHAR :
	ConvertBuffer( resSlice, FLOAT, &((u8*)resBuf)[z*resDim[0]*resDim[1]], 
		       resType, resDim[0]*resDim[1] );
	break;
      case SCHAR :
	ConvertBuffer( resSlice, FLOAT, &((s8*)resBuf)[z*resDim[0]*resDim[1]], 
		       resType, resDim[0]*resDim[1] );
	break;
      case USHORT :
	ConvertBuffer( resSlice, FLOAT, &((u16*)resBuf)[z*resDim[0]*resDim[1]], 
		       resType, resDim[0]*resDim[1] );
	break;
      case SSHORT :
	ConvertBuffer( resSlice, FLOAT, &((s16*)resBuf)[z*resDim[0]*resDim[1]], 
		       resType, resDim[0]*resDim[1] );
	break;
      }
    }
