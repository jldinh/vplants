...
\t -gradient-laplacian | -hessian | -gradient-hessian] [-pos|-neg]\n\
...
#endif
{
  ...
  case _FILTER_ :
    if ( RecursiveFilterOnBuffer( image->buf, image->type, 
				  ...
				  par.filterCoefs, par.filterType ) != 1 ) {
      VT_FreeImage( &imres ); 
      VT_FreeImage( image );
      VT_Free( (void**)&image );
  ...
  case _GRADIENT_EXTREMA_ :
    if ( Extract_Gradient_Maxima_2D( image->buf, image->type,
  ...
  case _GRADIENT_LAPLACIAN_ :
    if ( Gradient_On_Laplacian_ZeroCrossings_2D( image->buf, image->type,
		       imres.buf, imres.type,
		       theDims,
		       par.borderLengths,
		       par.filterCoefs,
  ...
  case _GRADIENT_HESSIAN_ :
    if ( Gradient_On_GradientHessianGradient_ZeroCrossings_2D( image->buf, image->type,
		       imres.buf, imres.type,
		       theDims,
		       par.borderLengths,
		       par.filterCoefs,
...
#endif
{
  ...
  while ( i < argc ) {
	...
	par->filterCoefs[2] = par->filterCoefs[1] = par->filterCoefs[0];
	par->filterType = GAUSSIAN_DERICHE;
      }
	...
	par->typeOutput = _GRADIENT_EXTREMA_;
      }
