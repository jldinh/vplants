...
static void WriteMatrice();
static int  ReadMatrice();
static void MatriceFromRealToVoxels();
...
{
  local_par par;
  vt_image *image, imres, *imdef[3], *imtemplate;
  vt_ipt resdim;
  double mat[16];
  ...
  case MODE_ISOTROPIC :
    ...
    mat[0] = par.voxel_isotropic / image->siz.x;
    mat[3] = 0.5 * (mat[0] - 1.0);
    mat[5] = par.voxel_isotropic / image->siz.y;
    mat[7] = 0.5 * (mat[5] - 1.0);
    mat[10] = par.voxel_isotropic / image->siz.z;
    mat[11] = 0.5 * (mat[10] - 1.0);

    resdim.x = (int)( (image->dim.x * image->siz.x / par.voxel_isotropic) + 1 );
    resdim.y = (int)( (image->dim.y * image->siz.y / par.voxel_isotropic) + 1 );
    resdim.z = (int)( (image->dim.z * image->siz.z / par.voxel_isotropic) + 1 );
  ...
  default :
    /* on construit une matrice d'homothetie, de rapport resdim.x/image->dim.x.
       ...
       */
    /* en fait, on construit la matrice inverse, pour le calcul */
    mat[0] = (double)image->dim.x / (double)resdim.x;
    mat[3] = 0.5 * (mat[0] - 1.0);
    mat[5] = (double)image->dim.y / (double)resdim.y;
    mat[7] = 0.5 * (mat[5] - 1.0);
    mat[10] = (double)image->dim.z / (double)resdim.z;
    mat[11] = 0.5 * (mat[10] - 1.0);
  }
...
static void WriteMatrice( char *name, double *mat )
#else
static void WriteMatrice( name, mat )
char *name;
double *mat;
#endif
{
...
static void MatriceFromRealToVoxels( double *mat, double *sizeIn, double *sizeOut )
#else
static void MatriceFromRealToVoxels( mat, sizeIn, sizeOut )
double *mat;
double *sizeIn;
double *sizeOut;
#endif
