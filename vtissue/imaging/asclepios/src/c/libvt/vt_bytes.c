...
#else
void VT_SwapImage( image )
vt_image *image; /* image to be swapped */ 
#endif
{
  switch ( image->type ) {
  case SSHORT :
...
#else
void VT_InverseImage( image )
vt_image *image; /* image to be inversed */ 
#endif
{
  int i;
...
#else 
void VT_ZeroImage( image )
vt_image *image; /* image to be filled with 0 */
#endif
{
  register int i, v;
...
#else
void VT_Swap2Octets( im1, im2 )
vt_image *im1; /* input image */
vt_image *im2; /* output image */
#endif
{
...
#endif
{
    ...
    if ( (im1->type != INT) && (im1->type != FLOAT) ) {
...
void VT_LogicInverse( vt_image *im1 /* output image */,
		      vt_image *im2 /* input image */ )
#else
void VT_LogicInverse( im1, im2 )
vt_image *im1; /* output image */
vt_image *im2; /* input image */
#endif
{
    register int i;
