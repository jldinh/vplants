...
#else
void VT_Histo( histo )
vt_histo *histo; /* histogram to be initialized */
#endif
{
    register int i;
    for (i=0; i<STRINGLENGTH; i++) histo->name[i] = '\0';
    histo->type = CPU_UNKNOWN;
    histo->size = 0;
    histo->offset = 0;
    histo->buf = (u64*)NULL;
...
#else
int VT_AllocHisto( histo )
vt_histo *histo; /* histogram to be allocated */
#endif
{
    register int i;
...
#else
int VT_ComputeHisto( histo, image )
vt_histo *histo; /* histogram to be allocated */
vt_image *image; /* input image */
#endif
{
...
int VT_ComputeMaskedHisto( vt_histo *histo /* histogram to be filled */,
			   ...
			   vt_image *mask )
#else
int VT_ComputeMaskedHisto( histo, image, mask )
vt_histo *histo; /* histogram to be allocated */
vt_image *image; /* input image */
vt_image *mask;
#endif
...
#else
int VT_CumulativeHisto( cum, histo )
vt_histo *cum;
vt_histo *histo; 
#endif
{
