...
#else
static int _Main_Computation();
static int _Local_Computation();
static void _Sort_CC();
static _my_cc * VT_RelabelLabelsImage();
static _my_cc * _LabelsOverflowManagement();
static void _SetOutputToBinary();
static void _UnsetOutputToBinary();
#endif
...
#else
int VT_ConnectedComponents( theIm, resIm, threshold, par )
vt_image *theIm;
vt_image *resIm;
float threshold;
vt_connexe *par;
...
int VT_Hysteresis( vt_image *theIm /* input image */, 
		   ...
		   vt_connexe *par /* parameters structure */ )
#else
int VT_Hysteresis( theIm, resIm, low, high, par )
vt_image *theIm;
vt_image *resIm;
float low;
float high;
...
int VT_ConnectedComponentWithOneSeed( vt_image *theIm /* input image */, 
				 ...
				 vt_connexe *par /* parameters structure */ )
#else
int VT_ConnectedComponentWithOneSeed( theIm, resIm, threshold, seed, par )
vt_image *theIm;
vt_image *resIm;
float threshold;
vt_ipt *seed;
...
#endif
{
  ...
  if ( (seed->x < 0) || (seed->x >= theIm->dim.x) ||
       (seed->y < 0) || (seed->y >= theIm->dim.y) ||
       (seed->z < 0) || (seed->z >= theIm->dim.z) ) {
...
int VT_ConnectedComponentsWithSeeds( vt_image *theIm /* input image */, 
				 ...
				 vt_connexe *par /* parameters structure */ )
#else
int VT_ConnectedComponentsWithSeeds( theIm, resIm, threshold, seeds, par )
vt_image *theIm;
vt_image *resIm;
float threshold;
vt_image *seeds;
...
static int _Main_Computation( vt_image *theIm /* pre-processed image */, 
			      vt_connexe *par /* parameters structure */ )
#else
static int _Main_Computation( theIm, par )
vt_image *theIm;
vt_connexe *par;
#endif
{
...
#else
static int _Local_Computation( theIm, par, theCc  )
vt_image *theIm;
vt_connexe *par;
_my_cc **theCc;
#endif
...
#else
int VT_CheckConnectivity( connexite, dimz )
...
static void _Sort_CC( _my_cc *tab, int left, int right )
#else
static void _Sort_CC( tab, left, right )
_my_cc *tab;
int left;
int right;
#endif
