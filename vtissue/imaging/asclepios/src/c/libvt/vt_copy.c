...
#else
int VT_CopyImage( theIm, resIm )
vt_image *theIm; /* image to be copied */
vt_image *resIm; /* image where the copy is to be done */
#endif
{
    ...
    if ( resIm->type == theIm->type ) {
      ...
      case INT : size = sizeof( i32 ); break;
...
#if defined(_ANSI_)
int VT_NormaImageWithInterval( vt_image *theIm, /* image to be copied */
			       vt_image *resIm,  /* image where the copy is to be done */
			       double min,
			       double max )
#else
int VT_NormaImageWithInterval( theIm, resIm, min, max )
vt_image *theIm; /* image to be copied */
vt_image *resIm; /* image where the copy is to be done */
double min;
double max;
#endif
{
	...
	register int z, s;
	vt_3m m;
	char *proc="VT_NormaImageWithInterval";
...
int VT_NormaMultImage( vt_image *theIm, /* image to be copied */
		   vt_image *resIm  /* image where the copy is to be done */ )
#else
int VT_NormaMultImage( theIm, resIm )
vt_image *theIm; /* image to be copied */
vt_image *resIm; /* image where the copy is to be done */
#endif
{
...
#else
int VT_CopyWindow( theIm, resIm, theCor, resCor, dim )
...
#endif
{
    ...
    if ( (theCor->x < 0) || (theCor->x >= theIm->dim.x) ||
	 (theCor->y < 0) || (theCor->y >= theIm->dim.y) ||
	 (theCor->z < 0) || (theCor->z >= theIm->dim.z) ) {
	VT_Error( "Upper left corner of the first image is not valid", "VT_CopyWindow" );
	return( 0 );
    }
    if ( (resCor->x < 0) || (resCor->x >= resIm->dim.x) ||
	 (resCor->y < 0) || (resCor->y >= resIm->dim.y) ||
	 (resCor->z < 0) || (resCor->z >= resIm->dim.z) ) {
    ...
    local_dim = *dim;
    if ( (theCor->x + local_dim.x - 1) >= theIm->dim.x )
	local_dim.x = theIm->dim.x - theCor->x;
    if ( (theCor->y + local_dim.y - 1) >= theIm->dim.y )
	local_dim.y = theIm->dim.y - theCor->y;
    if ( (theCor->z + local_dim.z - 1) >= theIm->dim.z )
	local_dim.z = theIm->dim.z - theCor->z;
    if ( (resCor->x + local_dim.x - 1) >= resIm->dim.x )
	local_dim.x = resIm->dim.x - resCor->x;
    if ( (resCor->y + local_dim.y - 1) >= resIm->dim.y )
	local_dim.y = resIm->dim.y - resCor->y;
    if ( (resCor->z + local_dim.z - 1) >= resIm->dim.z )
