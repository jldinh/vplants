...
__revision__="$Id: 08-03-11-2d,real,maxInInitium-withCenter.py 7875 2010-02-08 18:24:36Z cokelaer $"
...
create_tissue_topology_from_simulation( w, get_z_coords=False )
s = TissueSystem( w, const=WalledTissueDynamicCanalisation2() )
for c in s.tissue.cells():
    if len( s.tissue.cell2wvs( c ) )  < 3:
        s.tissue.remove_cell( c )
        print "removed: ", c
clear_incorrect_neighborhood( s.tissue )
from openalea.mersim.serial.merrysim_plugin import read_links
l = read_links("/home/stymek/mdata/07-07-05-PINwithImmunolab/c16-proj-grey.link")

z = pgl.Vector3()
for i in s.tissue.wvs():
    s.tissue.wv_pos( i, s.tissue.wv_pos( i )/100 ) 
    z += s.tissue.wv_pos( i )
z = z/ len(s.tissue.wvs())
for i in s.tissue.wvs():
    s.tissue.wv_pos( i, pos=s.tissue.wv_pos(i)-z)
for ce in l:
...
pgl.Viewer.camera.lookAt( (0,0,7.5), (0,0,0) )
...
def run():
    ...
    set_property_on_tissue_component(wt=s.tissue, cell=center_cell, f_component=pin_level, tol=1., property="C",
                                     property_value=True )
    set_property_on_tissue_component(wt=s.tissue, cell=p0, f_component=pin_level, tol=1., property="P0",
                                     property_value=True )
    set_property_on_tissue_component(wt=s.tissue, cell=p1, f_component=pin_level, tol=1., property="P1",
                                     property_value=True )
    set_property_on_tissue_component(wt=s.tissue, cell=p2, f_component=pin_level, tol=1., property="P2",
                                     property_value=True )
    
    s.update_visualisation( revers=False,pump_color=pgl.Color4(255,0,0,0), wall_color=pgl.Color4(0,0,0,0),
                           material_f= f_property2material("C"), prefix="real-c-")
    s.update_visualisation( revers=False,pump_color=pgl.Color4(255,0,0,0), wall_color=pgl.Color4(0,0,0,0),
                           material_f= f_property2material("P0"), prefix="real-p0-")
    s.update_visualisation( revers=False,pump_color=pgl.Color4(255,0,0,0), wall_color=pgl.Color4(0,0,0,0),
                           material_f= f_property2material("P1"), prefix="real-p1-")
    s.update_visualisation( revers=False,pump_color=pgl.Color4(255,0,0,0), wall_color=pgl.Color4(0,0,0,0),
                           material_f= f_property2material("P2"), prefix="real-p2-")
    ...
    s.update_visualisation( revers=False,pump_color=pgl.Color4(255,0,0,0), wall_color=pgl.Color4(0,0,0,0),
                           material_f= f_property2material("cC"), prefix="can-c-")
    s.update_visualisation( revers=False,pump_color=pgl.Color4(255,0,0,0), wall_color=pgl.Color4(0,0,0,0),
                           material_f= f_property2material("cP0"), prefix="can-p0-")
    s.update_visualisation( revers=False,pump_color=pgl.Color4(255,0,0,0), wall_color=pgl.Color4(0,0,0,0),
                           material_f= f_property2material("cP1"), prefix="can-p1-")
    s.update_visualisation( revers=False,pump_color=pgl.Color4(255,0,0,0), wall_color=pgl.Color4(0,0,0,0),
                           material_f= f_property2material("cP2"), prefix="can-p2-")
