...
__revision__="$Id: 08-01-31-2d,hex,lateralInhWithCenter-2.py 7875 2010-02-08 18:24:36Z cokelaer $"
    ...
    def f( self, x, t ):
        ...
        for i in t.cells():
            ...
            if not self.i_local_evacuate( i ):
                x2[ self.cell2sys[ i ] ] = self.c_sigma_a*(1.-0.3*self.i_center_evacuate( i )) - self.c_theta_a*A(i,t,x) 
...
#        self.phys=PhysAuxinTransportModel36( tissue_system=self )
    ...
    def mark_center_cells( self, cell ):
        ...
        self.tissue.cell_property( cell, "CC", True )
        for c in n1:
            self.tissue.cell_property( c, "CZ", True )
            self.tissue.cell_property( c, "CC", True )
    ...
    def run( self, nbr=30, nbr2=10 ):
        ...
        for i in self.tissue.cells():
            ...
            self.tissue.cell_property(i, "PrC", 0)
        
        cc = 1012
        self.mark_center_cells(cc)
        #assert False
        for i in range(nbr):
            self.phys.run()
            self.write_tissue("/home/stymek/mtmp/hex,3sinksWithCenter-2/"+str(self.frame))
            self.frame+=1
