...
__revision__="$Id: 07-10-07-canalisationWar.py 7875 2010-02-08 18:24:36Z cokelaer $"
...
class TissueSystem:
    ...
    def __init__( self, tissue, const=None ):
        self.tissue = tissue
        self.const=const
        self.phys=Phys2DcanalisationForDynamicTopView1( tissue_system=self )
    ...
    def step( self, nbr=1 ):
        for i in range( nbr ):
            ...
            self.update_visualisation(concentration_range=(0,3700))
    ...
    def save_simulation( self ):
        if self.c_save_simulation:
           import d
           import pickle
           save_path="/home/stymek/src/vplants/vplants/trunk/mersim/src/experiments/simualtion"
           d.write( self.tissue, name="s"+str(self.frame), path=save_path )
           pickle.dump( self.tissue._wv_edge2properties, file( save_path+"/"+"s"+str(self.frame)+".pickle", "w" ) )
    ...
    def update_visualisation( self, cell=None, **keys ):
        ...
        if keys.has_key( "pin_range" ): concentration_range = keys[ "pin_range" ]
        else: pin_range=(0.1,self.phys.c_pin_max)
        print "__", concentration_range
        ...
        #visualisation_pgl_2D_linear_tissue_aux_pin( s.tissue, concentration_range=s.concentration_range, pin_range=s.pin_range, max_wall_absolute_thickness=abs_intercellular_space+0.020, cell_marker_size=0.01, abs_intercellular_space=abs_intercellular_space )
        cz=self.cell_remover.c_remove_2d_radius
        pd.instant_update_viewer()
        name = ("%.5d.png") % self.frame
        pgl.Viewer.frameGL.saveImage(name)
        
    def update_visualisation2( self, cell=None, **keys ):
        self.frame+=1
        pd.SCENES[ 0 ].clear()
        self.update_conc_range()
        abs_intercellular_space = 0.01
...
#for i in wt3.cells():
#    wt3.cell_property( cell=i, property="auxin_level", value=15*calculate_cell_surface( wt, cell=cell ) )
#    if wtpr.cell_properties[ "auxin_level" ][ i ] < 0.3:
#        z+=1
#        wt3.cell_property( cell=i, property="PrC", value=z )
...
for i in s.tissue.cells():
    s.tissue.cell_property( cell=i, property="auxin_level", value=wtpr.cell_properties["auxin_level"][ i ] )
    s.tissue.cell_property( cell=i, property="BC", value=0 )
#for i in s.tissue.cell_edges():
#    s.phys.pin( i , wtpr.cell_properties["auxin_level"][ i ] )
for i in s.tissue.cells():
    if s.tissue.cell_property(i,"CZ"):
        s.phys.aux(i, 0 )   
#ss=pd.AISphere()
#ss.visible=False

import pickle
s.tissue._wv_edge2properties=pickle.load(file("pin-07-10-11.pickle", "r"))
