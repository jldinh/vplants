...
__revision__="$Id: 08-02-04-2d,long,vein.py 7875 2010-02-08 18:24:36Z cokelaer $"
...
class Phys(PhysAuxinTransportModel):
    ...
    def __init__(self, tissue_system = None):
        ...
        self.c_sigma_a_prim = 0.5 #creation of IAA
    ...
    def f( self, x, t ):
        ...
        for i in t.cells():
            ...
            if not self.i_sink( i ):
                x2[ self.cell2sys[ i ] ] = self.c_sigma_a*self.i_l1( i )+self.c_sigma_a_prim*self.i_source( i ) - self.c_theta_a*A(i,t,x) 
...
#        self.phys=PhysAuxinTransportModel36( tissue_system=self )
    ...
    def play1_1( self, folder="/home/stymek/mtmp/2dlongitudinalCut-1/"):
    ...
    def play3_1( self, folder="/home/stymek/mtmp/2dlongitudinalCut-3/"):
    ...
    def play3_2( self, folder="/home/stymek/mtmp/2dlongitudinalCut-3/"):
        green_range._position_list=[0.,0.7,1.]
        self.c_dynamic_aux=False
        self.c_disp_aux_max=5
        while True:
    ...
    def run( self, path, nbr=60, nbr2=10 ):
        self.phys.c_break_step_nbr=nbr2
        for i in range(nbr):
            self.phys.run()
            self.update_visualisation(abs_intercellular_space=0.004, max_wall_absolute_thickness=0.03,
                                      revers=True, material_f=auxin2material6, pump_color=pgl.Color4(255,0,0,0),
                                      wall_color=pgl.Color4(255,255,255,0),cell_marker_size=0.02)

            self.write_tissue(path+str(self.frame))
            #self.frame+=1

    def run2( self, nbr=60, nbr2=10 ):
        self.phys.c_break_step_nbr=nbr2
        for i in range(30):
            self.phys.run()
            self.write_tissue("/home/stymek/mtmp/2dlongitudinalCut-2/"+str(self.frame))
            self.frame+=1
        self.tissue.cell_property(184,"Source",1)
        for i in range(20):
            self.phys.run()
            self.write_tissue("/home/stymek/mtmp/2dlongitudinalCut-2/"+str(self.frame))
            self.frame+=1
        self.phys.c_sigma_a_prim=2.
        for i in range(20):
            self.phys.run()
            self.write_tissue("/home/stymek/mtmp/2dlongitudinalCut-2/"+str(self.frame))
            self.frame+=1
        self.phys.c_sigma_a_prim=4.
        for i in range(20):
            self.phys.run()
            self.write_tissue("/home/stymek/mtmp/2dlongitudinalCut-2/"+str(self.frame))
            self.frame+=1
        self.phys.c_sigma_a_prim=8.
        for i in range(20):
            self.phys.run()
            self.write_tissue("/home/stymek/mtmp/2dlongitudinalCut-2/"+str(self.frame))
            self.frame+=1
    ...
    def run3( self, nbr=60, nbr2=10 ):
        self.phys.c_phi_str = """def Phi( x ):
                                            if x < 0:
                                                return 0
                                            return 1.1*x*x"""
        self.phys.f_phi = pyfunction( self.phys.c_phi_str )
        self.phys.c_break_step_nbr=nbr2
        self.phys.c_sigma_a=0.2
        for i in range(nbr):
            self.phys.run()
            self.write_tissue("/home/stymek/mtmp/2dlongitudinalCut-3/"+str(self.frame))
            self.frame+=1
        self.tissue.cell_property(184,"Source",1)
        self.phys.c_sigma_a_prim=2.
        for i in range(20):
