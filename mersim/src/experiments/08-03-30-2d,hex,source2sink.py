...
__revision__="$Id: 08-03-30-2d,hex,source2sink.py 7875 2010-02-08 18:24:36Z cokelaer $"
...
#        self.phys=PhysAuxinTransportModel36( tissue_system=self )
    ...
    def update_visualisation( self, max_wall_absolute_thickness=0.5, cell_marker_size=0.5,
        ...
        #-----------
        max_wall_absolute_thickness=0.5
        cell_marker_size=0.5
        abs_intercellular_space=0.07
        #-----------
        ...
        keys[ "wall_color" ] = pgl.Color4(255,255,255,0)
        ...
        visualisation_pgl_2D_linear_tissue_aux_pin2( self.tissue, concentration_range=concentration_range, pin_range=pin_range, max_wall_absolute_thickness=max_wall_absolute_thickness,
                                                    ...
                                                    prim_cells_with_pin=self.phys.c_disp_pin_max, f_mark_regions=mark_regions2_a, **keys)
    ...
    def play1_2( self, folder="/home/stymek/mtmp/08-03-30-2d,hex,source2sink/"):
        green_range._position_list=[0.,0.7,1.]
        self.phys.c_dynamic_aux=False
        self.phys.c_disp_aux_max=6
        pgl.Viewer.frameGL.setBgColor(pgl.Color3(255,255,255))
        while True:
            self.load_tissue( folder+str(self.frame))
            self.update_visualisation(abs_intercellular_space = 0.07, max_wall_absolute_thickness=0.03, revers=True, material_f=auxin2material6, pump_color=pgl.Color4(255,0,0,0),
                                   prefix=folder+"vis/2/")
                    

    def run( self, nbr=60, nbr2=10, folder="/home/stymek/mtmp/08-03-30-2d,hex,source2sink/" ):
        ...
        for i in range(30):
            ...
            self.update_visualisation(abs_intercellular_space = 0.07, max_wall_absolute_thickness=0.03, revers=True, material_f=auxin2material6, pump_color=pgl.Color4(255,0,0,0),
                                   prefix=folder+"vis/1/")
            #self.frame+=1
        self.tissue.cell_property(s1,"Source",1)
        self.tissue.cell_property(s2,"Source",1)
        self.tissue.cell_property(s3,"Source",1)
        self.phys.c_sigma_a_prim=2.0
        for i in range(50):
            self.phys.run()
            self.write_tissue(folder+str(self.frame))
            self.update_visualisation(abs_intercellular_space = 0.07, max_wall_absolute_thickness=0.03, revers=True, material_f=auxin2material6, pump_color=pgl.Color4(255,0,0,0),
    ...
    def run_( self, nbr=60, nbr2=10, folder="/home/stymek/mtmp/08-03-30-2d,hex,source2sink/" ):
        self.phys.c_break_step_nbr=nbr2
        for i in range(nbr):
            self.phys.run()
            self.write_tissue(folder+str(self.frame))
...
sinks=[648,649,693,694]    
for i in sinks:
    s.tissue.cell_property( cell=i, property="Sink", value=1)

for i in [1332,1287,1243,1198,1154,1109,1065,1020]+range(1368,1377):
...
s.update_visualisation(abs_intercellular_space = 0.07, max_wall_absolute_thickness=0.03, revers=True, material_f=auxin2material6, pump_color=pgl.Color4(255,0,0,0), wall_color=pgl.Color4(255,255,255,0),
