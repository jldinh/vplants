...
__revision__="$Id: 07-10-12-canalisationWar-smallerTissue.py 7875 2010-02-08 18:24:36Z cokelaer $"
...
class TissueSystem:
    ...
    def __init__( self, tissue, const=None ):
        self.tissue = tissue
        self.const=const
        self.phys=Phys2DcanalisationForDynamicTopView2( tissue_system=self )
    ...
    def step( self, nbr=1 ):
        for i in range( nbr ):
            ...
            self.update_visualisation(concentration_range=(self.c_prim_max_con, 3500))
    ...
    def update_visualisation( self, cell=None, **keys ):
        ...
        pgl.Viewer.frameGL.saveImage(name)d
...
for i in s.tissue.cells():
    for pr in ["auxin_level", "PrC"]:
        s.tissue.cell_property( cell=i, property=pr, value=wtpr.cell_properties[pr][ i ] )
...
def p():
    print "cells:", len(s.tissue.cells())
    print "cell_edges:", len(s.tissue.cell_edges())
    print "wvs", len(s.tissue.wvs())


s.load_simulation( filename="s00148" )   
s.update_conc_range()
s.update_pin_range()
s.update_visualisation(concentration_range=s.concentration_range)

# serching for better pin fit
for i in s.tissue.cells():
    if s.tissue.cell_property( i, "PrZ"):
        s.phys.aux( i, 0 )
...
#s.play_simulation()
    ...
    ksi=5e-7
    change=1
    s.phys.c_break_cond = 10000

def start():
    s.phys.step()
    s.save_simulation( filename="ksi_5e-7")
    s.update_conc_range()
    filename="ksi_5e-7"
    s.update_visualisation2( concentration_range=s.concentration_range, pin_range=(0.00005,0.00008),filename=filename+".png")
    #s.play_simulation()
    #
    s.phys.c_ksi = 7.5e-7
    s.phys.h=step/5
    s.phys.c_change = s.phys.c_change/5
    s.phys.step()
    s.save_simulation( filename="ksi_7.5e-7")
    s.update_conc_range()
    filename="ksi_7.5e-7"
    s.update_visualisation2( concentration_range=s.concentration_range, pin_range=(0.00005,0.00008),filename=filename+".png")
    ...
    s.phys.c_ksi = 1e-6
    s.phys.h=step/10
    s.phys.c_change = s.phys.c_change/10
    s.phys.step()
    s.save_simulation( filename="ksi_1e-6")
    s.update_conc_range()
    filename="ksi_1e-6"
    s.update_visualisation2( concentration_range=s.concentration_range, pin_range=(0.00005,0.00008),filename=filename+".png")

    s.phys.c_ksi = 1.25e-6
    s.phys.h=step/15
    s.phys.c_change = s.phys.c_change/15
    s.phys.step()
    s.save_simulation( filename="ksi_1.25e-6")
    s.update_conc_range()
    filename="ksi_1.25e-6"
    s.update_visualisation2( concentration_range=s.concentration_range, pin_range=(0.00005,0.00008),filename=filename+".png")
    #
