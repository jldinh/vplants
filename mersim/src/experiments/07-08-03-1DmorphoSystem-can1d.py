...
__revision__="$Id: 07-08-03-1DmorphoSystem-can1d.py 7875 2010-02-08 18:24:36Z cokelaer $"
...
class TissueSystem:
    ...
    def __init__( self, tissue, const=None ):
        self.tissue = tissue
        self.const=const
        self.phys=Phys1Dcanalisation1( tissue_system=self )
    ...
    def update_visualisation( self, cell=None ):
        ...
        visualisation_pgl_1D_linear_tissue_aux_pin(self.tissue, concentration_range=self.concentration_range, pin_range=(0,4), max_wall_absolute_thickness=2., cell_marker_size=1.)
        cz=self.cell_remover.c_remove_2d_radius
        pd.instant_update_viewer()
        pgl.Viewer.frameGL.saveImage("r"+str(self.frame)+".png")

        visualisation_pgl_1D_linear_tissue_aux_pin(self.tissue, concentration_range=self.concentration_range, pin_range=self.pin_range, max_wall_absolute_thickness=2., cell_marker_size=1.)
        cz=self.cell_remover.c_remove_2d_radius
        pd.instant_update_viewer()
...
wtpr = WalledTissuePickleReader( "/home/stymek/mdata/1DroundTissue50", mode="r")
wtpr.read_tissue()
...
#s.tissue.cell_property( 24, "CZ", 1)
s.phys.init_aux()
...
pgl.Viewer.camera.set( (0,10,30), -90, -90 )
