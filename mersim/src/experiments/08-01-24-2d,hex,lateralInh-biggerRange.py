...
__revision__="$Id: 08-01-24-2d,hex,lateralInh-biggerRange.py 7875 2010-02-08 18:24:36Z cokelaer $"
...
#        self.phys=PhysAuxinTransportModel36( tissue_system=self )
    ...
    def play1( self, folder):
        ...
        while True:
            ...
            self.update_visualisation(abs_intercellular_space=0.05, revers=True, material_f=auxin2material6)
            #self.frame+=1
    ...
    def play4( self, folder):
        while True:
            self.load_tissue( folder+str(self.frame))
            #self.update_visualisation()
            self.update_visualisation(abs_intercellular_space=0.05, revers=True, material_f=auxin2material3, pump_color=pgl.Color4(0,0,0,0), wall_color=pgl.Color4(255,255,255,0))
            #self.frame+=1

    def play5( self, folder):
        while True:
            self.load_tissue( folder+str(self.frame))
            #self.update_visualisation()
            self.update_visualisation(abs_intercellular_space=0.05, revers=True, material_f=auxin2material3, pump_color=pgl.Color4(255,255,255,0), wall_color=pgl.Color4(0,0,0,0))
            #self.frame+=1

    def play6( self, folder):
        while True:
            self.load_tissue( folder+str(self.frame))
            #self.update_visualisation()
            self.update_visualisation(abs_intercellular_space=0.05, revers=True, material_f=auxin2material7, pump_color=pgl.Color4(255,0,0,0), wall_color=pgl.Color4(255,255,255,0))
            #self.frame+=1

    def play7( self, folder):
        green_range._position_list=[0.,0.9,1.]
    ...
    def run( self, nbr=30, nbr2=10 ):
        ...
        for j in range(3):
            self.mark_prim_cells(prims[j][0],prims[j][1])            
            for i in range(nbr):
                self.phys.run()
                self.write_tissue("/home/stymek/mtmp/hex,3sinks-biggerRange-1/"+str(self.frame))
                self.frame+=1

    def run2( self, nbr=80, nbr2=10 ):
        p0 = 648
        p1 = 1376
        p2 = 838
        prims = [(p0,1),(p1,2),(p2,3)]
        ...
        for j in range(3):
            ...
            for i in range(nbr):
                ...
                self.write_tissue("/home/stymek/mtmp/hex,3sinks-biggerRange-2/"+str(self.frame))
                self.frame+=1

    def temp(self):
        self.run2()
        self.play2_1("/home/stymek/mtmp/hex,3sinks-2/")
