...
__revision__="$Id: 08-02-19-2d,hex,veinAndWound.py 7875 2010-02-08 18:24:36Z cokelaer $"
...
class Phys(PhysAuxinTransportModel):
    ...
    def __init__(self, tissue_system = None):
        ...
        ########################################################
        #### equation constants
        self.c_sigma_a = 0.2 #creation of IAA
        self.c_sigma_a_prim = 3.0 #creation of IAA
    ...
    def f( self, x, t ):
        ...
        for (i,j) in t.cell_edges():
            x2[ self.wall2sys[(i,j)] ] = self.f_phi(J((i,j),t,x))+self.c_sigma_p-self.f_fixed_pin(P((i,j),t,x))*self.c_theta_p
            x2[ self.wall2sys[(j,i)] ] = self.f_phi(J((j,i),t,x))+self.c_sigma_p-self.f_fixed_pin(P((j,i),t,x))*self.c_theta_p
        return x2

    def f_fixed_pin( self, x):
        if x>0.95*self.c_max_PIN:
            return 0.001*x
        else: return x
...
#        self.phys=PhysAuxinTransportModel36( tissue_system=self )
    ...
    def play1( self, folder="/home/stymek/mtmp/08-02-19-2d,hex,veinAndWound/"):
    ...
    def run0( self, nbr=30, nbr2=10, folder="/home/stymek/mtmp/08-02-19-2d,hex,veinAndWound/" ):
        self.phys.c_break_step_nbr=nbr2
        for i in range(nbr):
            self.phys.run()
            self.write_tissue("/home/stymek/mtmp/08-02-19-2d,hex,veinAndWound/"+str(self.frame))
            self.update_visualisation(abs_intercellular_space=0.05, revers=True, material_f=auxin2material6, wall_color=pgl.Color4(255,255,255,0),
                                      prefix=folder+"vis/1/")
            self.frame+=1

            
    def run( self, nbr=30, nbr2=10, folder="/home/stymek/mtmp/08-02-19-2d,hex,veinAndWound-2/" ):
        self.phys.c_break_step_nbr=nbr2
        ...
        self.tissue.cell_property( cell=1376, property="Source", value=1)
        
        self.phys.c_sigma_a_prim=1.
        for i in range(5):
            self.phys.run()
            self.write_tissue(folder+str(self.frame))
            self.update_visualisation(abs_intercellular_space=0.05, revers=True, material_f=auxin2material6, wall_color=pgl.Color4(255,255,255,0),
        ...
        for i in [1020,1065,1019,975,1109,1064,1018,974,929,884,928,973,1017,1063,1108,1154]:
            self.tissue.remove_cell(i)
    
    def run2( self, nbr=30, nbr2=10, folder="/home/stymek/mtmp/08-02-19-2d,hex,veinAndWound-2/" ):
        self.phys.c_sigma_a_prim=2.
        for i in range(15):
            self.phys.run()
            self.write_tissue(folder+str(self.frame))
...
s.load_tissue( "/home/stymek/mtmp/08-02-19-2d,hex,veinAndWound/45")#45/" )
s.frame=46
s.phys.c_sigma_a_prim=2.
sinks=range(648,657)    
for i in sinks:
    s.tissue.cell_property( cell=i, property="Sink", value=1)
...
s.tissue.cell_property( cell=1376, property="Source", value=1)

s.const.cell_properties["Sink"]=0
s.const.cell_properties["Source"]=0
s.const.cell_properties["L1"]=0
for i in [1020,1065,1019,975,1109,1064,1018,974,929,884,928,973,1017,1063,1108,1154,1016,1015]:
    s.tissue.remove_cell(i)
s.phys.prepare_sys_map()


# --- positioning to real meristem.
pgl.Viewer.camera.setOrthographic()
pgl.Viewer.frameGL.setSize(1200,1200)
pgl.Viewer.camera.set( (0,0,32), -90, -90 )
# --- end of positioning to real meristem.
s.phys.c_dynamic_aux=False
#s.load_tissue("/home/stymek/mtmp/hex,3sinks-1/0")
s.update_visualisation(abs_intercellular_space=0.05, revers=True, material_f=auxin2material3, pump_color=pgl.Color4(255,255,255,0), wall_color=pgl.Color4(0,0,0,0))
#s.frame=0

#s.phys.c_dynamic_aux=True