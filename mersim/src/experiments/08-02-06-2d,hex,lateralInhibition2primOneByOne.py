...
__revision__="$Id: 08-02-06-2d,hex,lateralInhibition2primOneByOne.py 7875 2010-02-08 18:24:36Z cokelaer $"
    ...
    def f( self, x, t ):
        ...
        for i in t.cells():
            ...
            if not self.i_local_evacuate( i ):
                x2[ self.cell2sys[ i ] ] = self.c_sigma_a*(1.-0.1*self.i_center_evacuate( i )) - self.c_theta_a*A(i,t,x) 
...
#        self.phys=PhysAuxinTransportModel36( tissue_system=self )
    ...
    def play1( self, folder="/home/stymek/mtmp/08-02-06-2d,hex,lateralInhibition2primOneByOne/"):
        green_range._position_list=[0.,0.94,1.]
        self.c_dynamic_auxin=False
        self.c_disp_aux_max=5
        while True:
            ...
            self.update_visualisation(abs_intercellular_space=0.05, revers=True, material_f=auxin2material6, wall_color=pgl.Color4(255,255,255,0),
                                      prefix="/home/stymek/mtmp/08-02-06-2d,hex,lateralInhibition2primOneByOne/vis/1/")
            #self.frame+=1

    def play2( self, folder="/home/stymek/mtmp/08-02-06-2d,hex,lateralInhibition2primOneByOne/"):
        green_range._position_list=[0.,0.94,1.]
        self.phys.c_dynamic_auxin=False
        self.phys.c_disp_aux_max=5
        while True:
            self.load_tissue( folder+str(self.frame))
            #self.update_visualisation()
            self.update_visualisation(abs_intercellular_space=0.05, revers=True, material_f=auxin2material6, wall_color=pgl.Color4(255,255,255,0),
                                      prefix="/home/stymek/mtmp/08-02-06-2d,hex,lateralInhibition2primOneByOne/vis/2/")
            #self.frame+=1
    ...
    def run( self, nbr=60, nbr2=10 ):
        ...
        for i in range(nbr):
            self.phys.run()
            self.write_tissue("/home/stymek/mtmp/08-02-06-2d,hex,lateralInhibition2primOneByOne/"+str(self.frame))
            self.frame+=1

        self.mark_prim_cells(prims[1][0],prims[1][1])            
