...
__revision__="$Id: 08-02-01-2d,real,maxInInitium-withCenterOverProducing.py 7875 2010-02-08 18:24:36Z cokelaer $"
...
class Phys(PhysAuxinTransportModel):
    ...
    def __init__(self, tissue_system = None):
        ...
        self.c_sigma_a_prim = 0.05 #creation of IAA
    ...
    def f( self, x, t ):
        ...
        for i in t.cells():
            ...
            if not self.i_local_evacuate( i ):
                x2[ self.cell2sys[ i ] ] = self.c_sigma_a +self.i_center_evacuate( i )*self.c_sigma_a_prim - self.c_theta_a*A(i,t,x) 
...
#        self.phys=PhysAuxinTransportModel36( tissue_system=self )
    ...
    def play1( self, folder="/home/stymek/mtmp/08-02-01-2d,real,maxInInitium-withCenterOverProducing/"):
        ...
        while True:
            ...
            self.update_visualisation(max_wall_absolute_thickness=0.05,cell_marker_size=0.05,
                                      abs_intercellular_space=0.01,revers=False,
                                      material_f=auxin2material6, pump_color=pgl.Color4(255,0,0,0),
                                      wall_color=pgl.Color4(255,255,255,0), prefix=folder+"vis/1/")
            #self.frame+=1
            
    def run( self, nbr=60, nbr2=10 ):
        cc = 346
        p0 = 460
        p1 = 258
        p2 = 170
        ...
        green_range._position_list=[0.,0.94,1.]
        self.phys.h=0.25
        self.phys.c_sigma_a_prim=0.3

        self.write_tissue("/home/stymek/mtmp/08-02-01-2d,real,maxInInitium-withCenterOverProducing/"+str(self.frame))
        self.frame+=1
        for i in prims:
        ...
        for i in range(nbr):
            self.phys.run()
            self.update_visualisation(max_wall_absolute_thickness=0.05,cell_marker_size=0.05,abs_intercellular_space=0.01,revers=True, material_f=auxin2material6, pump_color=pgl.Color4(255,0,0,0), wall_color=pgl.Color4(0,0,0,0))
            self.write_tissue("/home/stymek/mtmp/08-02-01-2d,real,maxInInitium-withCenterOverProducing/"+str(self.frame))
            #self.frame+=1
...
#s.load_tissue("/home/dasilva/mtmp/hex,3sinks-1/0")
