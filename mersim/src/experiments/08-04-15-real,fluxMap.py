...
__revision__="$Id: 08-04-15-real,fluxMap.py 8407 2010-03-08 07:53:28Z pradal $"
...
#        self.phys=PhysAuxinTransportModel36( tissue_system=self )
    ...
    def mark_center_cells( self, cell, iaa_level=0 ):
        ...
        for c in n1:
            ...
            for n in self.tissue.cell_neighbors( c ):
                ...
                for nn in self.tissue.cell_neighbors( c ):
                    self.tissue.cell_property( nn, "CZ", True )
                    auxin_level( self.tissue, nn, 0 )
        try:
    ...
    def play1( self, folder="/home/stymek/mtmp/08-03-14-pinDynBiggerCenter-1/" ):
        ...
        while True:
            ...
            self.tissue.tissue_system=self
            self.tissue.cell2flux = self.cell2flux_f()
            print min(self.tissue.cell2flux.values()), max(self.tissue.cell2flux.values())
            self.update_visualisation(max_wall_absolute_thickness=0.06,cell_marker_size=0.05,abs_intercellular_space=0.01,revers=False, material_f=flux2material, pump_color=pgl.Color4(255,0,0,0), wall_color=pgl.Color4(0,0,0,0),
                                      prefix="/home/stymek/mtmp/08-04-15-real,fluxMap/"+"vis/1/")
            #self.frame+=1
    ...
    def run( self, nbr=60, nbr2=10, folder="/home/stymek/mtmp/08-04-15-real,fluxMap/" ):
        cc = 346
        p0 = 460
        p1 = 258
        p2 = 170
        ...
        self.mark_center_cells(322, 1)
        self.mark_center_cells(353, 1)
        self.mark_center_cells(323, 1)
        self.mark_center_cells(250, 1)
        self.mark_center_cells(519, 1)
        self.mark_center_cells(313, 1)
        #for i in self.tissue.cells():
...
#import pickle
#cz = pickle.load(file("/home/stymek/mtmp/08-04-15-real,fluxMap/CZ.pickle"))
#for i in cz:
...
s.frame=60
s.load_tissue( "/home/stymek/mtmp/08-03-14-pinDynBiggerCenter-1/"+str(s.frame))
s.tissue.tissue_system=s
s.tissue.cell2flux = s.cell2flux_f()
print min(s.tissue.cell2flux.values()), max(s.tissue.cell2flux.values())
s.phys.c_dynamic_aux=False
#s.phys.c_disp_aux_max=0.03
s.phys.c_disp_aux_max=1
s.tissue.c_flux_factor=100
s.tissue.c_concentration_factor=1
visualisation_conf["concentration_range"]=[0,12]
s.update_visualisation(max_wall_absolute_thickness=0.06,cell_marker_size=0.05,abs_intercellular_space=0.01,revers=False, material_f=fluxAndAuxin2material, pump_color=pgl.Color4(255,0,0,0), wall_color=pgl.Color4(0,0,0,0),
                                      prefix="/home/stymek/mtmp/08-04-15-real,fluxMap/"+"vis/1/")
