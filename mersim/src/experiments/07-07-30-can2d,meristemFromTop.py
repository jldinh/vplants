...
__revision__="$Id: 07-07-30-can2d,meristemFromTop.py 7875 2010-02-08 18:24:36Z cokelaer $"
...
class TissueSystem:
    ...
    def __init__( self, tissue, const=None ):
        self.tissue = tissue
        self.const=const
        self.phys=PhysAuxinInternalModel1( tissue_system=self )
    ...
    def update_visualisation( self, cell=None ):
        ...
        #visualisation3( self.tissue, concentration_range=self.concentration_range, pin_range=self.pin_range )
        #visualisation_of_flux_zones_1(s.tissue,[cell]+self.tissue.cell_neighbors(cell),self.tol, pin_range=self.pin_range)
        visualisation_pgl_2D_linear_tissue_aux_pin( self.tissue, concentration_range=self.concentration_range, pin_range=self.pin_range )
...
#    s.tissue.cell_property( cell=i, property="auxin_level", value=1.)#*calculate_cell_surface(s.tissue, i) )
#for i in [p0,p1,p2]:
