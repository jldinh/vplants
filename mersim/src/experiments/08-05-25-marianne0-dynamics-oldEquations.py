...
__revision__="$Id: 08-05-25-marianne0-dynamics-oldEquations.py 7875 2010-02-08 18:24:36Z cokelaer $"
...
class TissueSystem:
    ...
    def __init__( self, tissue, const=None ):
        ...
        #self.c_tissue_dir="/home/stymek/mtmp/08-02-28-can2d,dynamic,spiralPhyllotaxis3/"
    ...
    def run( self , nbr):
        # trial to do alternate pattern:
        #self.c_tissue_dir="/home/stymek/mtmp/08-03-01-can2d,dynamic,spiralPhyllotaxis3/"
        ...
        self.phys.h=1.
        self.phys.c_prim_trs=7.5
        self.phys.c_primordium_fade_out_conc_max=6
        self.phys.c_primordium_fade_out_time=1000
        self.phys.init_aux()
        self.phys.init_pin()
        self.phys.c_dynamic_aux=True
        self.step(nbr,prefix=self.c_tissue_dir)
        
    def run1( self , nbr):
        #self.c_tissue_dir="/home/stymek/mtmp/08-03-01-can2d,dynamic,spiralPhyllotaxis3-1/"
        self.phys.c_stable_tolerance=0.01
        self.phys.c_atol=0.001
        self.phys.c_rtol=0.001
        self.phys.c_sigma_a=0.3
        ...
        self.phys.c_prim_trs=7.5
        self.phys.c_primordium_fade_out_conc_max=6
        self.phys.c_primordium_fade_out_time=600
        self.phys.init_aux()
        self.phys.init_pin()
        self.phys.c_dynamic_aux=True
        self.step(nbr,prefix=self.c_tissue_dir)
    ...
    def step( self, nbr=1, prefix="" ):
        for i in range( nbr ):
            ...
            self.update_visualisation( revers=True, material_f=auxin2material3, pump_color=pgl.Color4(255,0,0,0), wall_color=pgl.Color4(0,0,0,0), prefix=prefix+"vis/1/")
            self.growth.apply()
    ...
    def update_visualisation( self, max_wall_absolute_thickness=0.08, cell_marker_size=0.02,
                             abs_intercellular_space=0.01, revers=True, material_f=auxin2material5,
                             prefix="", **keys ):
        #self.frame+=1
        pd.SCENES[ 0 ].clear()
        self.update_conc_range()
        ...
        visualisation_pgl_2D_linear_tissue_aux_pin2( self.tissue, concentration_range=concentration_range, pin_range=pin_range, max_wall_absolute_thickness=max_wall_absolute_thickness,
                                                    ...
                                                    prim_cells_with_pin=self.phys.c_disp_pin_max, stride=20, f_mark_regions=mark_regions3, **keys)
    ...
    def reverse_tissue_if_needed( self ):
        ...
        return not(0 < (pgl.cross(self.tissue.wv_pos(cwvs[0]) - cc, self.tissue.wv_pos(cwvs[1]) - cc)).z)
        
    def reverse_cells_if_needed( self ):
        for c in self.tissue.cells():
            cwvs = self.tissue.cell2wvs(c)
            cc = cell_center( self.tissue, c)
            #print  (pgl.cross(s.tissue.wv_pos(cwvs[0]) - cc, s.tissue.wv_pos(cwvs[1]) - cc)).z
            if not(0 < (pgl.cross(self.tissue.wv_pos(cwvs[0]) - cc, self.tissue.wv_pos(cwvs[1]) - cc)).z):
                l=self.tissue._cell2wv_list[ c ]
                l.reverse()
                self.tissue._cell2wv_list[ c ] = l
...
def run():
    ...
    #visualisation_conf["material_f"]=f_property2material("cC")
    visualisation( t, visualisation_conf )
    pgl.Viewer.frameGL.saveImage("08-05-21-marianne01-mersim-real.png")
    tol=0.1001
    for i in cc:
        set_property_on_tissue_component(wt=s.tissue, cell=i, f_component=pin_level, tol=tol, property="cC",
    ...
    set_property_on_tissue_component(wt=s.tissue, cell=p2, f_component=pin_level, tol=tol, property="cP2",
                                     property_value=True )
    set_property_on_tissue_component(wt=s.tissue, cell=p3, f_component=pin_level, tol=tol, property="cP3",
                                    property_value=True )
    set_property_on_tissue_component(wt=s.tissue, cell=p4, f_component=pin_level, tol=tol, property="cP4",
                                    property_value=True )
    set_property_on_tissue_component(wt=s.tissue, cell=p5, f_component=pin_level, tol=tol, property="cP5",
                                    property_value=True )
    visualisation_conf["material_f"]=f_property2material("cC")
    visualisation( t, visualisation_conf )
    pgl.Viewer.frameGL.saveImage("08-05-21-marianne01-mersim-can-cC.png")
    visualisation_conf["material_f"]=f_property2material("cP0")
    visualisation( t, visualisation_conf )
    pgl.Viewer.frameGL.saveImage("08-05-21-marianne01-mersim-can-cP0.png")
    visualisation_conf["material_f"]=f_property2material("cP1")
    visualisation( t, visualisation_conf )
    pgl.Viewer.frameGL.saveImage("08-05-21-marianne01-mersim-can-cP1.png")
    visualisation_conf["material_f"]=f_property2material("cP2")
    visualisation( t, visualisation_conf )
    pgl.Viewer.frameGL.saveImage("08-05-21-marianne01-mersim-can-cP2.png")
    visualisation_conf["material_f"]=f_property2material("cP3")
    visualisation( t, visualisation_conf )
    pgl.Viewer.frameGL.saveImage("08-05-21-marianne01-mersim-can-cP3.png")
    visualisation_conf["material_f"]=f_property2material("cP4")
    visualisation( t, visualisation_conf )
    pgl.Viewer.frameGL.saveImage("08-05-21-marianne01-mersim-can-cP4.png")
    visualisation_conf["material_f"]=f_property2material("cP5")
    visualisation( t, visualisation_conf )
    pgl.Viewer.frameGL.saveImage("08-05-21-marianne01-mersim-can-cP5.png")
