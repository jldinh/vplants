...
__revision__="$Id: 08-03-25-2d,long,separetedLayers.py 7875 2010-02-08 18:24:36Z cokelaer $"
...
class Phys(PhysAuxinTransportModel):
    ...
    def __init__(self, tissue_system = None):
        ...
        self.c_sigma_a_prim = 0.5#5 #creation of IAA
        self.c_sigma_a_bis = 0.01 #creation of IAA
        self.c_theta_a = 0.01 #destruction of IAA
        self.c_theta_a_prim = 0.1 #destruction of IAA
        ...
        self.c_phi_str2 = """def Phi( x ):
                                            if x < 0:
                                                return 0
                                            return 0.09*x#0.11*x"""
        self.f_phi2 = pyfunction( self.c_phi_str2 )
        self.c_PIN_capping_enabled=True
        self.c_max_PIN=2.
        self.c_sink_fixed_concentration=True
        self.c_sink_destruction_rate=0.1
        self.c_leak_from_l1_coef = 1.#0.01
    ...
    def f( self, x, t ):
        ...
        for i in t.cells():
            ...
            if not self.i_sink( i ):
                x2[ self.cell2sys[ i ] ] = self.c_sigma_a_bis+self.c_sigma_a*self.i_l1( i )+self.c_sigma_a_prim*self.i_source( i ) - (self.c_theta_a+self.c_theta_a_prim*self.i_L1sink(i))*A(i,t,x) 
                if self.i_l1( i ) and not self.i_L1bridge(i):
                    # you are L1 cell but NOT bridge cell
                    for n in t.cell_neighbors(i):
                        if self.i_l1( n ): x2[ self.cell2sys[ i ] ] += J((n,i),t,x) - J((i,n),t,x)
                        else: x2[ self.cell2sys[ i ] ] += 0#self.c_leak_from_l1_coef*(J((n,i),t,x) - J((i,n),t,x))
                elif self.i_L1bridge(i):
                    # you are bridge cell
                    for n in t.cell_neighbors(i):
                    ...
                    # you are inner cell
                    for n in t.cell_neighbors(i):    
                        if self.i_l1( n ) and not self.i_L1bridge(n):
                            x2[ self.cell2sys[ i ] ] += 0
                        else:
                            x2[ self.cell2sys[ i ] ] += J((n,i),t,x) - J((i,n),t,x)
                ...
                #        else: x2[ self.cell2sys[ i ] ] += 0#self.c_leak_from_l1_coef*(J((n,i),t,x) - J((i,n),t,x))
                #    else:
                #        if not self.i_l1( i ) or self.i_L1bridge( i ):
                #            x2[ self.cell2sys[ i ] ] += J((n,i),t,x) - J((i,n),t,x)
                #        else:
                #            x2[ self.cell2sys[ i ] ] += 0
        ...
        for (i,j) in t.cell_edges():
            if (self.i_l1( i ) or self.i_l1( j )) and not(self.i_L1bridge(i) or self.i_L1bridge(j)):
                if self.i_l1( i ):
                    if self.i_l1( j ): 
                        x2[ self.wall2sys[(i,j)] ] = self.f_phi2(J((i,j),t,x))+self.c_sigma_p-self.c_theta_p*P((i,j),t,x)
                    else:
                        x2[ self.wall2sys[(i,j)] ] = self.c_sigma_p-self.c_theta_p*P((i,j),t,x)
                (i,j)=(j,i)
                if self.i_l1( i ):
                    if self.i_l1( j ): 
                        x2[ self.wall2sys[(i,j)] ] = self.f_phi2(J((i,j),t,x))+self.c_sigma_p-self.c_theta_p*P((i,j),t,x)
    ...
    def i_sink( self, cell ):
        t = self.system.tissue
        if t.cell_property( cell=cell, property="Sink"):
        #if self.h_prim_cells[ cell ]:
            return 1
        else:
            return 0

    def i_L1sink( self, cell ):
        t = self.system.tissue
        if t.cell_property( cell=cell, property="L1Sink"):
        #if self.h_prim_cells[ cell ]:
            return 1
        else:
            return 0

    def i_L1bridge( self, cell ):
        t = self.system.tissue
        if t.cell_property( cell=cell, property="L1Bridge"):
        #if self.h_prim_cells[ cell ]:
            return 1
        else:
            return 0
    ...
    def i_l1( self, cell ):
        t = self.system.tissue
        if t.cell_property( cell=cell, property="L1") > 0:
        #if self.h_centre_cells[ cell ]:
            return 1
        else:
            return 0

    def i_source( self, cell ):
        t = self.system.tissue
        if t.cell_property( cell=cell, property="Source") > 0:
        #if self.h_centre_cells[ cell ]:
            return 1
        else:
            return 0
...
#        self.phys=PhysAuxinTransportModel36( tissue_system=self )
    ...
    def update_visualisation( self, max_wall_absolute_thickness=0.5, cell_marker_size=0.5,
        ...
        visualisation_pgl_2D_linear_tissue_aux_pin3( self.tissue, concentration_range=concentration_range, pin_range=pin_range, max_wall_absolute_thickness=max_wall_absolute_thickness,
    ...
    def load_tissue( self, file_name ):
        ...
        wtpr.read_property( "Sink" )
        wtpr.read_property( "Source" )
        wtpr.read_property( "L1" )
        for i in wt3.cells():
            ...
            self.tissue.cell_property( cell=i, property="Source", value=wtpr.cell_properties[ "Source" ][ i ] )
            self.tissue.cell_property( cell=i, property="Sink", value=wtpr.cell_properties[ "Sink" ][ i ] )
            self.tissue.cell_property( cell=i, property="L1", value=wtpr.cell_properties[ "L1" ][ i ] )
    ...
    def play( self, folder="/home/stymek/mtmp/08-03-25-2d,long,separetedLayers/"):
        ...
        while True:
            ...
            self.update_visualisation(abs_intercellular_space=0.004, max_wall_absolute_thickness=0.03,
                                      revers=True, material_f=auxin2material6, pump_color=pgl.Color4(255,0,0,0),
                                      wall_color=pgl.Color4(255,255,255,0),cell_marker_size=0.02, prefix=folder+"vis/1/")

                    

    def run( self, nbr=60, nbr2=10, folder="/home/stymek/mtmp/08-03-25-2d,long,separetedLayers/" ):
        self.write_tissue(folder+str(self.frame))
        self.frame+=1
        self.phys.c_break_step_nbr=nbr2
        self.phys.c_sigma_a=0.2
        self.tissue.cell_property(165,"Sink",1)
        self.tissue.cell_property(182,"Sink",1)
        self.phys.c_sigma_a_prim=0.
        self.phys.c_dynamic_aux=True
        auxin_level( s.tissue, 165, 0)
        auxin_level( s.tissue, 182, 0)
        for i in range(30):
            self.phys.run()
            self.write_tissue(folder+str(self.frame))
            self.update_visualisation(abs_intercellular_space=0.004, max_wall_absolute_thickness=0.03,
                                      ...
                                      wall_color=pgl.Color4(255,255,255,0),cell_marker_size=0.02, prefix=folder+"vis/1/")
            #self.frame+=1
        #self.tissue.cell_property(165,"Source",1)
        #self.tissue.cell_property(170,"Source",1)
        #self.tissue.cell_property(176,"Source",1)
        #self.tissue.cell_property(182,"Source",1)
        #self.tissue.cell_property(187,"Source",1)
        self.tissue.cell_property(173,"Source",1)
        self.tissue.cell_property(173,"L1Bridge",1)
        ...
        s.phys.c_sigma_a_prim=1.
        s.run_(5)
        s.phys.c_sigma_a_prim=1.6
        s.run_(5)
        s.run_(5)
        s.run_(5)
        s.run_(5)
    ...
    def run_( self, nbr=60, nbr2=10, folder="/home/stymek/mtmp/08-03-25-2d,long,separetedLayers/" ):
        for i in range(nbr):
            self.phys.run()
            self.write_tissue(folder+str(self.frame))
            self.update_visualisation(abs_intercellular_space=0.004, max_wall_absolute_thickness=0.03,
    ...
    def run2( self, nbr=60, nbr2=10, folder="/home/stymek/mtmp/08-03-25-2d,long,separetedLayers-2/" ):
        self.write_tissue(folder+str(self.frame))
        self.frame+=1
        self.phys.c_break_step_nbr=nbr2
    ...
    def play2_2( self, folder="/home/stymek/mtmp/08-03-25-2d,long,separetedLayers-2/"):
        green_range._position_list=[0.,0.8,1.]
        self.phys.c_dynamic_aux=False
        self.phys.c_disp_aux_max=10.
        pgl.Viewer.frameGL.setBgColor(pgl.Color3(255,255,255))
        while True:
            self.load_tissue( folder+str(self.frame))
            self.update_visualisation(abs_intercellular_space=0.004, max_wall_absolute_thickness=0.03,
                                      revers=True, material_f=auxin2material6, pump_color=pgl.Color4(255,0,0,0),
                                      wall_color=pgl.Color4(255,255,255,0),cell_marker_size=0.02, prefix=folder+"vis/2/")

    def play2_3( self, folder="/home/stymek/mtmp/08-03-25-2d,long,separetedLayers-2/"):
        green_range._position_list=[0.,0.6,1.]
        self.phys.c_dynamic_aux=False
        self.phys.c_disp_aux_max=12.
        pgl.Viewer.frameGL.setBgColor(pgl.Color3(255,255,255))
        while True:
            self.load_tissue( folder+str(self.frame))
            self.update_visualisation(abs_intercellular_space=0.004, max_wall_absolute_thickness=0.03,
                                      revers=True, material_f=auxin2material6, pump_color=pgl.Color4(255,0,0,0),
                                      wall_color=pgl.Color4(255,255,255,0),cell_marker_size=0.02, prefix=folder+"vis/3/")

wtpr = WalledTissuePickleReader( "/home/stymek/mdata/2dTransversalCutExported-1", mode="r")
wtpr.read_tissue()
...
for i in s.tissue.cells():
    s.tissue.cell_property( cell=i, property="Sink", value=0)
    s.tissue.cell_property( cell=i, property="Source", value=0)
    s.tissue.cell_property( cell=i, property="L1", value=0)
    s.tissue.cell_property( cell=i, property="L1Sink", value=0)
    s.tissue.cell_property( cell=i, property="L1Bridge", value=0)

#Sinks=[144,91,84,82,119,71,54,132,80,145,154,47,57]
sinks=[144,91,84,82,119]    
for i in sinks:
    s.tissue.cell_property( cell=i, property="Sink", value=1)

for i in range(160,192):
    s.tissue.cell_property( cell=i, property="L1", value=1)

s.const.cell_properties["Sink"]=0
s.const.cell_properties["Source"]=0
s.const.cell_properties["L1"]=0
s.const.cell_properties["L1Bridge"]=0
s.const.cell_properties["L1Sink"]=0

recomended_source=184    
...
s.update_visualisation(abs_intercellular_space=0.004, max_wall_absolute_thickness=0.03, revers=True, material_f=auxin2material6, pump_color=pgl.Color4(255,0,0,0), wall_color=pgl.Color4(255,255,255,0),
                       cell_marker_size=0.02)
s.frame=0