...
"""
import visual
from openalea.mersim.tissue.algo.walled_tissue import cell_center, cell_centers 
...
class SimulationRemoveCellStrategy2D_1(SimulationRemoveCellStrategy):
    ...
    def __init__( self, system=None, c_remove_2d_radius=450):
        ...
        for i in self.system.tissue.cells():
            v += cell_center( self.system.tissue, i )
        self.center = v / len(self.system.tissue.cells())
        self.c_remove_2d_radius=c_remove_2d_radius
        
    def cells_to_remove(self):
        """Returns cells  to remove acording to curtent strategy.
        """
        l = []
        cc=cell_centers( self.system.tissue )
        for c in self.system.tissue.cells():
            if visual.mag( cc[ c ] - self.center) > self.c_remove_2d_radius:
                l.append( c )
        return l
...
class SimulationRemoveCellStrategy2D_2(SimulationRemoveCellStrategy):
    ...
    def __init__( self, system=None):
        ...
        ids of cells which are observed to remove.
        """
        SimulationRemoveCellStrategy.__init__( self, system=system, c_remove_dist=2000)
        v =  visual.vector()
        self.c_remove_dist = c_remove_dist
        for i in self.system.tissue.cells():
            v += cell_center( self.system.tissue, i )
        self.center = v / len(self.system.tissue.cells())
    ...
    def cells_to_remove(self):
        ...
        cc=cell_centers( self.system.tissue )
        for c in self.system.tissue.cells():
            if visual.mag( cc[ c ] - self.center) > self.c_remove_dist:
                l.append( c )
