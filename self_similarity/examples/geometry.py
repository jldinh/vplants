...
def compute_geom_parameters(pf, vtxlist, verbose = True, get_father = Father,scale=3):
    ...
    rel_orientations = dict([ (i , rel_orientation(pf,i)) for i in VtxList(Scale=scale) ])
...
def compute_representation(root, get_parameters, get_sons = Sons, get_father = Father):
    ...
    while len(vstack) > 0:
        ...
        t.setWidth(get_parameters(v,'Diameter')*5)
